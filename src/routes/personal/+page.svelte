...
<div class="container mx-auto max-w-md px-4 py-8 lg:max-w-7xl">
	<div class="flex flex-col gap-8 lg:flex-row">
		<!-- 왼쪽 섹션: 프로필 이미지 및 기본 정보 -->
		<div class="lg:w-1/3">
			<div class="sticky top-8">
				<div class="mb-6 flex flex-col items-center">
					<div class="relative mb-4 h-56 w-40 lg:h-72 lg:w-56">
						<img
						...
						{#if $profile.isEditMode}
							<label
								for="profileImage"
								class="bg-primary absolute right-2 bottom-2 cursor-pointer rounded-full p-2 text-white"
							>
								<svg
					...
					<div class="w-full text-center">
						<h1 class="mb-1 text-2xl font-bold sm:text-3xl">{$profile.name} {$profile.position}</h1>
						<p class="mb-6 text-base sm:text-lg">{$profile.motto}</p>
							...
							<div class="mt-6 grid grid-cols-3 gap-4">
								<div class="text-center">
									<div class="text-2xl font-bold lg:text-3xl">
										{$profile.stats?.caseCount || 0}+
									</div>
									<div class="text-muted-foreground text-xs lg:text-sm">사건</div>
								</div>
								<div class="text-center">
									<div class="text-2xl font-bold text-emerald-500 sm:text-3xl">
										{$profile.stats?.winRate || 0}%
									</div>
									<div class="text-muted-foreground text-xs sm:text-sm">승소율</div>
								</div>
								<div class="text-center">
									<div class="text-2xl font-bold sm:text-3xl">
										{$profile.stats?.yearCount || 0}y
									</div>
									<div class="text-muted-foreground text-xs sm:text-sm">경력</div>
								</div>
							</div>
						</div>
		...
		<!-- 오른쪽 섹션: 탭 및 콘텐츠 -->
		<div class="mt-8 sm:mt-0 sm:w-2/3">
			<Tabs value={currentTab} class="w-full">
				...
				<TabsContent value="profile" class="mt-4 space-y-6">
					<Card class="p-4 sm:p-6">
						<h2 class="mb-2 text-xl font-semibold sm:mb-4 sm:text-2xl">경력</h2>
						<CareerSection />
					</Card>

					<Card class="p-4 sm:p-6">
						<h2 class="mb-2 text-xl font-semibold sm:mb-4 sm:text-2xl">학력</h2>
						<EducationSection />
					</Card>

					<Card class="p-4 sm:p-6">
						<h2 class="mb-2 text-xl font-semibold sm:mb-4 sm:text-2xl">자격</h2>
						<CertificatesSection />
					</Card>

					<Card class="p-4 sm:p-6">
						<h2 class="mb-2 text-xl font-semibold sm:mb-4 sm:text-2xl">전문 분야</h2>
						<SpecialtiesSection />
					</Card>
				</TabsContent>
